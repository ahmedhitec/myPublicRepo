...
(function ( $, util, debug, server, lang, actions, region, theme, message, item, model, widgetUtil ) {
     ...
     * to assist in customizing the toolbar. See {@link toolbar} widget for details on the structure of the toolbar
     * data. See option {@link interactiveGrid#toolbarData} and method {@link interactiveGrid.copyDefaultToolbar}
     ...
     * </table>
     * </p>
     * @property {function} toolbarFind(id) Returns a control group, control or menu item found by id.
     ...
     * @property {function} toolbarInsertAfter(id, item) Inserts the given item after the id if found. The id
     *     argument can be the id of a control group, control, or menu item or an action name. If the id is
     ...
     * @property {function} toolbarRemove(id) Removes a control group, control or menu item found by id.
    ...
    $.widget( "apex.interactiveGrid",
            ...
            if ( lConfig.parentRegionStaticId && lConfig.trackParentSelection ) {
                ...
                $( "#" + lConfig.parentRegionStaticId + "_ig" ).on( "interactivegridselectionchange.ig", util.debounce( function ( event, data ) {
                    ...
                    if ( data.selectedRecords.length === 1 ) {
                        self.actions.enable( "selection-add-row" );
                        rec = data.selectedRecords[0];
                    } else {
                        // when there isn't exactly 1 master record selected there is an empty model and it
                        // is not possible to add a row so disable the add row button.
                        self.actions.disable( "selection-add-row" );
                    }
                    self.setMasterRecord( data.model, rec );
                }, 300 ) );
            }
        ...
        _registerView: function( pView ) {
            var self = this,
                ...
                clearModelData: function() { // clear the model to cause the view to get fresh data from the server
                    // there is no point in clearing the model if it is the place holder detail model where there
        ...
        _renderDownloadDialog: function() {
            ...
            this._createDialog( DOWNLOAD, function( renderItem ) {
                ...
                load: function( pModel ) {
                    var lInitFormat, lPrintOrientation, lPrintPageSize;

                    if ( lDownloadConfig && lDownloadConfig.formats.length > 0 ) {
                        lInitFormat = lDownloadConfig.formats[ 0 ];
                    }
                ...
                init: function( pModel, pNav$, pRecordView$ ) {
                    ...
                    pRecordView$.on( "apexbeginrecordedit", function() {
                        ...
                        lFormat$.change( function (event, data) {
                            pModel.setValue( pRecordView$.recordView( "getRecord" ), "format", $(this).val());
                            if ( $(this).val() === "PDF" ) {
                                showFields( [lPrintPageSize$,lPrintOrientation$,lPrintAccessible$] );
                            } else {
                                hideFields( [lPrintPageSize$,lPrintOrientation$,lPrintAccessible$] );
                            }
                        } );
                        ...
                        if ( lViewImpl.modelColumns.hasOwnProperty( key ) ) {
                            ...
                            if ( lViewImpl.modelColumns[ key ].id ) {
                                ...
                                if ( this._isColumnFilterable( lViewImpl.modelColumns[ key ].id ) ) {
                                    ...
                                    if ( $.inArray( this._getColumn(lViewImpl.modelColumns[ key ].id).filter.dataType, [ VARCHAR2, CLOB ] ) !== -1 ) {
                                        // does it support the CONTAINS operator?
                                        for ( i = 0; i < this._getColumn(lViewImpl.modelColumns[ key ].id).filter.operators.length; i++ ) {
                                             if ( $.inArray( this._getColumn(lViewImpl.modelColumns[ key ].id).filter.operators[i].name, [ "$perform$", "C" ] ) !== -1 ) {
       ...
       // todo use css to add the '...'
                ...
                if ( lReportSettings.reportSettingsArea === init ) {
                    ...
                    self._setReportSettings({}, function () {
                        ...
                        if ( lFocusOnControllingElement ) {
                            self._getElement( "report_settings" ).data( "apex-collapsible" ).controllingElement$.focus();
                        }
                    });
        ...
        _focusOnSettingsCheckbox: function ( pFocus, pId, pType ) {
            ...
            if ( pFocus ) {
                ...
                this._getElement( "report_settings" ).find( lCondition ).focus();
            }
        },
        ...
        _defaultSavedReport: function( pSavedReport ) {
            ...
            if ( !lConfigViews[ lNewReport.currentView ] || lConfigViews[ lNewReport.currentView ].disabled ) {
                ...
                for ( i in lConfigViews ) {
                    if ( lConfigViews.hasOwnProperty( i ) && lConfigViews[ i ] && !lConfigViews[ i ].disabled ) {
        ...
        setMasterRecord: function( pMasterModel, pMasterRecord ) {
            ...
            // Store the parentModel id, we'll it when setting up the new model
            this.parentModel = pMasterModel.modelId();

            if ( pMasterRecord ) {
                this._setModelInstance( pMasterModel.getRecordId( pMasterRecord ), pMasterModel, pMasterRecord );
            } else {
    ...
    var GRID_RANGE_ACTIONS = [ "selection-fill", "selection-clear", "selection-copy-down" ],
                ...
                if ( lEditable ) {
                    ...
                    if ( lModelColumn.defaultValue === undefined ) {
                        ...
                        if ( $.isPlainObject( defaultValue ) &&
                            ...
                            defaultValue.d = item( lModelColumn.elementId ).displayValueFor( defaultValue.d );
                        }
        ...
        initView: function ( pOptions ) {
            var i, lIGColumnGroup, lCustomToolbar, lStickyTop, lStickyFooter, lGridOptions, lReportAggs,
                ...
                lRowHeader = lConfig.defaultGridViewOptions.rowHeader || NONE,
                lIg = pOptions.ig,
            ...
            switch ( lConfig.headingFixedTo ) {
                ...
                case PAGE:
                    ...
                    lStickyFooter = lConfig.defaultGridViewOptions.stickyFooter || true;
                    break;
            }
        ...
        setModelName: function( pModelName ) {
            this.curInst.option( "modelName", pModelName );
        },
        setEditMode: function( pMode ) {
    ...
    registerView( $.extend( {}, baseIconAndDetailView, {
        ...
        canCopy2Clipboard: function() {
            ...
            // iconList.allowCopy
            var o = this.view$.tableModelView( "option", "iconListOptions" );
    ...
    registerView( {
        ...
        setColumnConfig: function( pOptions ) {
            ...
            for ( lMapping = 0; lMapping < lColumnMappings.length; lMapping++ ) {
                ...
                if ( lChartColumns.hasOwnProperty( lChartColumn ) ) {
