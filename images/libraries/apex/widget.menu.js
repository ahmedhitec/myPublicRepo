...
(function ( util, debug, lang, $ ) {
            ...
            if ( typeof item.action === "string" && actions ) {
                ...
                if ( item.action ) {
                    ...
                    if ( item.target ) {
                        ...
                        apex.navigation.openInNewWindow( item.href, item.target, {favorTabbedBrowsing: true, noopener: true} );
                    } else {
     ...
     * @property {string} [target] The target window for the URL href. Only applies when href is provided. Typically "_blank" to open in a new tab or window.
        ...
        open: function ( x, y ) {
            ...
            function finish() {
                ...
                if ( menu.customContent ) {
                    ...
                    out.clear();
                    owns = self._renderMenu( out, idprefix, menu, isPopup, true );
...
//                        .attr( A_OWNS, owns);
        ...
        _parseMenuMarkup: function ( $el, isMenubar ) {
            ...
            $el.children( "ul" ).children( "li" ).each( function( i ) {
                var item, icon, index, id, customId, customContent$, action, target,
                    $item = $( this ),
        ...
        _parseCustomMarkup: function( $menu, idprefix, menu ) {
            ...
            $menu.find( SEL_FOCUSABLE ).each( function(index) {
                var menuId,  item, item$, f$, href, target;
                f$ = $( this );
                ...
                if ( this.nodeName === "A" && href ) {
                    item = { type: "action", label: f$.text(), href: href };
        ...
        _renderMenubar: function ( out, idprefix, menubar ) {
            ...
            $.each( menubar.items, function( i, item ) {
                ...
                if ( type === "action" && typeof item.action === "string"  ) {
                    if ( !apex.actions ) {
...
// see above comment about aria-owns
                                //.optionalAttr( A_OWNS, isSubMenu ? id + "m" : null )
            ...
            $.each( menu.items, function ( i, item ) {
                ...
                if ( ( item.type === "action" || item.type === "toggle" || item.type === "radioGroup" ) &&
                    ...
                    if ( action ) {
                        ...
                        if ( action.shortcut && !item.accelerator ) {
                            accelerator = apex.actions.shortcutDisplay( action.shortcut );
                        }
                        if ( action.hide !== undefined ) {
                            item.hide = action.hide;
                        }
                ...
                if ( type === "radioGroup" ) {
                    ...
                    if ( type === "action" || type === "display" ) {
                        ...
                        subOwns = self._renderMenu( subOut, id, item.menu, false, false );
...
//                            .optionalAttr( A_OWNS, subOwns )
     ...
     * <code class="prettyprint">aria-haspopup="menu"</code> and <code class="prettyprint">aria-expanded="false"</code>
    ...
    $( function() {
        ...
        $( SEL_MENUBUTTON ).attr( A_HASPOPUP, "menu" ).attr( A_EXPANDED, "false" );

        // if APEX refreshes a region then fix up any new menu buttons
        $( "body" ).on( "apexafterrefresh", function( event ) {
            $( event.target ).find( SEL_MENUBUTTON ).attr( A_HASPOPUP, "menu" ).attr( A_EXPANDED, "false" );
        } );
