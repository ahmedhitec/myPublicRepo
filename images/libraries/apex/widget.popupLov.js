...
(function( widget, item, util, $ ) {
     ...
     * @param {string} initialFetch One of "none", "empty", "value". Determines what if anything will be fetched
        ...
        itemInstance = {
            ...
            refresh: function() {
                ...
                if ( options.dynamicDefault ) {
                    // xxx todo ask server for value and display value
                    setTimeout( function () {
                        theItem.setValue( "TODO", "To do" );
                    }, 900);
                } else {
            ...
            setValue: function( value, displayValue ) {
                var i, values, items, displayValues;

                if ( options.multiple ) {
                    values = util.toArray( value, options.separator );
                    if ( displayValue ) {
                        displayValues = util.toArray( displayValue, options.separator );
                    }
                    // clear old values
                    input$.val( "" );
                    ...
                    if ( !displayValue && value === options.nullValue ) {
                        displayValue = options.nullDisplayValue;
                    }
