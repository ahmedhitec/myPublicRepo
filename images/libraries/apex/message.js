...
( function( message, $, util, lang ) {
     ...
     * Is one of:
     *  - simple string message no markup allowed
     ...
     * @param {boolean} [options.isPopup] If true a popup widget is opened. If false a dialog widget is opened.
     *     Default is false.
     ...
     * @param {string} [options.okLabelKey] Message key for the OK button label.
     * @param {string} [options.cancelLabelKey] Message key for the cancel button label.
    ...
    function showDialog( messageContent, options ) {
        var dialog$, dialogOptions, // could be a popup or dialog widget
        ...
        if ( dialog$ && dialog$[0] ) {
            ...
            if ( typeof messageContent === "string" ) {
                dialog$ = jQuery( "<div><p>" + util.escapeHTML( messageContent ).replace( /\r\n|\n/g, "<br>" ) + "</p></div>" );
            ...
            if ( options.okButton ) {
                buttons.unshift( {
                    class: "js-confirmBtn",
                    text: lang.getMessage( options.okLabelKey ),
                    click: function () {
                        result = true;
                        dialog$[widget]( "close" );
                    }
            ...
            if ( options.confirm ) {
                buttons.unshift( {
                    text: lang.getMessage( options.cancelLabelKey ),
                    click: function () {
                        result = false;
                        dialog$[widget]( "close" );
                    }
            ...
            dialogOptions = {
                ...
                create: function () {
                    ...
                    // make OK button hot
                    okBtn$ = dialog$.parent().find( ".js-confirmBtn" ).addClass( "ui-button--hot" );
                },
                open: function ( event ) {
    ...
    $( function() {
        ...
        $( "#APEX_ERROR_MESSAGE" )
            ...
            .on( "click", "a.a-Notification-link", function( pEvent ) {
                var lRegion, lRegionId, lItem, lApexItem,
                    lLink$ = $( this ),
                    lErrorContext = {};
