...
(function ( server, util, debug, $ ) {
            ...
            if ( o.navigation && nodeAdapter.getLink && !event.isDefaultPrevented() ) {
                ...
                if ( href ) {
                    ...
                    if ( target ) {
                        ...
                        apex.navigation.openInNewWindow( href, target, {favorTabbedBrowsing: true, noopener: true} );
                    } else {
     ...
     * @property {string} [linkTarget] The target window used to open a link returned by {@link treeNodeAdpater#getLinkTarget}.
     ...
     *              linkTargetField - default linkTarget,
    ...
    $.apex.treeView.makeModelNodeAdapter = function( modelId, options, data ) {
        ...
        that._linkTargetKey = that.getFieldKey( options.linkTargetField || "linkTarget" );
    ...
    apex.widget.tree = {
        init: function( pTreeId, pStaticData, pHasIdentity, pOptions ) {
            ...
            if ( pOptions.regionStaticId && pOptions.lazyLoading ) {
                // only lazy load tree if it's visible at page load 
                // otherwise load tree when onVisibilityChange returns true
                setTimeout( function () {
                    if ( !isTreeVisible( tree$ ) ) {
                        apex.widget.util.onVisibilityChange( tree$[ 0 ], function ( show ) {
                            if ( show ) {
                                initAjaxTree( tree$, pOptions, pOptions.ajaxIdentifier, pOptions.itemsToSubmit );
                                apex.widget.util.offVisibilityChange( tree$[ 0 ] );
                            }
                        });
                    } else {
                        initAjaxTree( tree$, pOptions, pOptions.ajaxIdentifier, pOptions.itemsToSubmit );
            ...
            function getSelectedNodes( pTree$ ) {
                var selectedNodes = pTree$.treeView( "getSelectedNodes" );
            ...
            function setExpandedNodeIds( pTree$, pExpandedNodeIds ) {
                ...
                if ( pExpandedNodeIds ) {
                    for ( var i = 0; i < pExpandedNodeIds.length; i++ ) {
                        sel$ = pTree$.treeView("find", {
                            depth: -1,
                            findAll: false,
                            match: function ( node ) {
            ...
            function isTreeVisible( pTree$ ) {
                var tabRegion$,
                    activeTab$,
                    isVisible = true,
                    isInTab = ( pTree$ && pTree$.closest( "div.apex-tabs-region" ).length > 0 );

                if ( pTree$ ) {
                    isVisible = pTree$.is(':visible');
                }

                // separate test for APEX tabs needed, because of timing issues
                if ( isInTab ) {
                    tabRegion$ = apex.region( pTree$.closest( "div.apex-tabs-region" ).attr( "id" ) ).widget();

                    if ( tabRegion$ ) {
                        activeTab$ = tabRegion$.aTabs( "getActive" ).el$;
                        if ( pTree$.parents( "#" + activeTab$.attr( "id" ) ).length === 0 ) {
                            isVisible = false;
                        }
                    }
                }
...
})( apex.server, apex.util, apex.debug, apex.jQuery );
