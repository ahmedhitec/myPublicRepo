...
(function ( apex, debug, lang, util, $ ) {
    ...
    var mapCodeToName = (function() {
        ...
        for ( k in map ) {
            if ( map.hasOwnProperty( k ) ) {
                map[k] = map[k] === 1 ? k.replace( /Arrow|Digit|Key/, "" ) : map[k];
            }
        }
        return map;
    })();
        ...
        var actionsPrototype = {
            ...
            addFromMarkup: function( pList$ ) {
                ...
                pList$.find( "ul" ).eq(0).children( "li" ).each(function() {
                    var action, icon, index, existingAction, target,
                        title = null,
                        $item = $( this ),
                        a$ = $item.find( "a" ).eq( 0 ), // dig a little deeper for custom content menus
                        span$ = $item.find( "span.a-Menu-label" ).eq( 0 );  // xxx consider how to customize the a-Menu-label?
                ...
                if ( a && ( a.action || a.href ) ) {
                    if ( !a.disabled && !a.hide ) {
                        if ( a.action ) {
                            ...
                            // it must be href
                            if ( a.target ) {
                                apex.navigation.openInNewWindow( a.href, a.target, {favorTabbedBrowsing: true} );
                            } else {
             ...
             */
            shortcutDisplay: function( pShortcutName, pWithMarkup ) {
                var i, kb, ke, pb, pe,
                    display = "",
    ...
    apex.actions.setKeyCaps = function( pKeyCaps ) {
        ...
        if ( pKeyCaps === null ) {
            ...
            for ( k in gKeyCaps ) {
                gKeyCaps[k] = util.stripHTML( gKeyCaps[k] );
            }
        }
    };
