...
( function ( da, server, item, $, region, message ) {
    "use strict";
    ...
    da.showAlert = function() {
        var cb = this.resumeCallback;
        message.alert( apex.util.applyTemplate( this.action.attribute01 ), function() {
            da.resume( cb, false );
        } );
    }; // showAlert
    ...
    da.askConfirm = function() {
        var cb = this.resumeCallback;
        message.confirm( apex.util.applyTemplate( this.action.attribute01 ), function( ok ) {
            if ( !ok ) {
...
})( apex.da, apex.server, apex.item, apex.jQuery, apex.region, apex.message );