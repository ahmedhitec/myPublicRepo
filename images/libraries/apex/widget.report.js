...
( function( report, server, $, debug, markdown ) {
    "use strict";
    ...
    /**
     * In case of any of the columns in the report is marked as markdown type
     * this function renders all its elements marked with '.js-apex-markdown'
     * class.
     * Check reports2.plb file for markup details.
     */
    function renderMarkdownColumns( reportId ) {
        if ( markdown ) {
            markdown.render( reportId + ' .js-apex-markdown' );
        }
    }
    ...
    report.init = function( pRegionId, pAjaxIdentifier, pOptions ) {
        var lInternalRegionId, facetsRegion;
        ...
        if ( pOptions.apexFacets ) {
            gFacetsMap[ lInternalRegionId ] = pOptions.apexFacets;
            $(function() {
                facetsRegion = apex.region( pOptions.apexFacets );
        ...
        renderMarkdownColumns( "#" + pRegionId );
    ...
    function _refresh ( pId, pAjaxIdentifier, pData ) {
        ...
        p = server.plugin ( pAjaxIdentifier, lData, lOptions );
        p.always( function() {
            if ( facetsRegion ) {
                facetsRegion.unlock();
            }
...
})( apex.widget.report, apex.server, apex.jQuery, apex.debug, apex.widget.markdown );
