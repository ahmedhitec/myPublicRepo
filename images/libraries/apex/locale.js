...
( function( locale, debug, $ ) {
    ...
    var gLocaleSettings  = {   // locale depending settings
            language: "en",
            separators: {
                group:   ",",
                decimal: "."
    ...
    var loadJETcldrResources = function( resourcesPath, language) {
        var url, ajaxOptions, p1, p2, allLocales, curLang,
        ...
        // todo consider maybe any use of require such as for Monaco editor should include the the JET l10n config
        // todo as is you can't use code editor and JET or apex.locale.formatCompactNumber on the same page
        if ( window.require && window.define ) {
            ...
            // xxx can't move this condition in to outer if because of fake define in else branch
    ...
    locale.init = function( pOptions ) {
        ...
        $(function() {
            var resourcesPath = gLocaleSettings.resources;
            if ( resourcesPath ) {
    ...
    */
    ...
    locale.formatNumber = function( pValue, pFormat, pOptions ) {
        var s, d, absVal, m, cur, sign, fm, b, numFmt, index, leadingCur, zeroPad, upCase, strNum, width, fmtWidth, src,
        ...
        if ( pFormat == null ) { // intentional ==
            return "" + pValue; // no formatting just make it a string.
        }
    ...
    locale.formatCompactNumber = function( pValue, pOptions ) {
        var result, number, numberSettings, lang, root, optSep, pattern, parts, gs, gs0,
        ...
        numberSettings = {
            ...
            plurals: root.supplemental.plurals,
...
})( apex.locale, apex.debug, apex.jQuery );
