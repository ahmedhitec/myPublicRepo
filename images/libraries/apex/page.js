...
(function( page, $, event, message, server, util, lang ) {
"use strict";
...
function validPageItem( name, type ) {
    var valid = server.isValidPageItemName( name );
        ...
        lRunSubmitProcessing = function() {
                 ...
                 $( "#pPageItemsProtected", apex.gPageContext$ ).length === 0 )
            {
                ...
                if ( lNumTimeouts > 5 ) {
                    ...
                    message.alert( lang.getMessage( "APEX.WAIT_UNTIL_PAGE_LOADED" ) );
                } else {
                ...
                if ( lOptions.validate ) {
                    if ( !page.validate( lForm$ ) ) {
                        lPageValid = false;
                        message.alert( lang.getMessage( "APEX.CORRECT_ERRORS" ), function() {
...
page.bindTemplates = function( el$ ) {
    ...
    el$.find( ".js-bindContent .js-bindContent" ).each( function () {
        apex.debug.error( "Nested template content bindings not allowed." );
        $( this ).attr( "class", "" );
    } );

    el$.find( ".js-bindContent" ).each( function () {
        var depends, targetEl$, node,
            changeSel = "",
            keySel = "",
            removeClasses = false,
            body$ = $( "body" ),
        ...
        function process() {
            var focusedId, text = apex.util.applyTemplate( templateText ),
        ...
        var debounceProcess = util.debounce( process, 150 );
    ...
    el$.find( ".js-bindClass" ).each( function () {
        ...
        function process() {
            var text = apex.util.applyTemplate( templateText );
...
})( apex.page, apex.jQuery, apex.event, apex.message, apex.server, apex.util, apex.lang );
