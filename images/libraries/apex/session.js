...
(function( session, message, navigation, lang, debug, $ ) {
    "use strict";

    var gWarningSeconds, gRedirectUrl, gIdleWarningTimeoutId, gIdleExpiredTimeoutId, gMaxWarningTimeoutId, gMaxExpiredTimeoutId,
                ...
                if ( pNewTimeoutData.idle_time_ms === 0 ) {
                    ...
                    _showTimeoutAlert( lang.getMessage( "APEX.SESSION.ALERT.EXPIRED" ), {
                        ...
                        callback:   function( okPressed ) {
                            if ( okPressed ) {
                                navigation.redirect ( gRedirectUrl );
                            }
                        }
                    });
                } else {
     ...
     * todo doc
     */
    session.initTimeouts = function( pWarningSeconds, pTimeoutData, pRedirectUrl ) {
        gWarningSeconds = pWarningSeconds;
        gRedirectUrl = pRedirectUrl;
...
})(apex.session, apex.message, apex.navigation, apex.lang, apex.debug, apex.jQuery);
