 ...
 */
/*
 * There are 2 kind of scroll pagination.
 * 1) Add more (aka high-water mark) scrolling. As you scroll near to the end of the report more content is added.
 * This can be automatic or have a "load more" button.
 * 2) Virtual scrolling. In this case filler content is added so that it looks like all the data is in the report
 * (you can see this by looking at the proportional scroll bar thumb size). As you scroll the filler is replaced with
 * report data. In theory data could also be removed from the DOM when it is far outside the viewport to save memory
 * but currently this is not done.
 * Which kind of scroll pagination is used depends on the model option hasTotalRecords. If true then virtual scrolling
 * is done. If false add more scrolling is done.
...
(function ( util, model, debug, lang, item, $ ) {
    ...
    // todo consider this belongs in locale ns
    function toPlainNumber(value, formatMask) {
        var re,
            locale = apex.locale;

        if ( typeof value !== "number" ) {
            re = new RegExp( util.escapeRegExp( locale.getCurrency() ) + "|" +
                util.escapeRegExp( locale.getDualCurrency() ) + "|" +
                locale.getISOCurrency() + "|" +
                util.escapeRegExp( locale.getGroupSeparator() ), "g" );
            value = value.replace( re, "" );
            if ( locale.getDecimalSeparator() !== "." ) {
                value.replace( locale.getDecimalSeparator(), "." );
            }
        }
        return value;
    }
    ...
    $.widget( "apex.tableModelViewBase",
         ...
         * scrollPaging         true when a scroll handler will be used to do paging. Not exactly the same as
         *                      pagination.scroll. for example will be false when doing load more scroll paging.
         * footer$
         * pageRange$
         * pageSelector$
         * status$
        ...
        _tableModelViewDestroy: function() {
            ...
            if ( this._scrollHandler ) {
                this.scrollParent$.off( "scroll", null, this._scrollHandler );
            }
        },
        ...
        _paginationHasFocus: function() {
            ...
            if ( activeElement$.closest( "." + C_GRID_FOOTER ).length > 0 ) {
                if ( activeElement$.parent().hasClass( "a-GV-pageSelector-item" ) ) {
                    return activeElement$.parent().attr( "data-value" );
                } else {
        ...
        _restorePaginationFocus: function( prevFocus ) {
            ...
            if ( prevFocus ) {
                sel = prevFocus;
                if ( sel >= 0 ) {
                    sel = "[data-value='" + prevFocus + "'] button";
                }
        ...
        _addPageOfRecords: function( callback ) {
            var count, pageSize, pageOffset, dataContainer$, data$, insertControlBreak,
            ...
            function finish() {
                var filler$, outFiller, total, recPerRow, prevTotal,
                    frozenFiller$ = null,
                    hasTotalRecords = self.model.getOption( "hasTotalRecords" ),
                ...
                if ( self.pageOffset === 0 && count === 0) {
                    ...
                    if ( self.scrollPaging && hasTotalRecords ) {
                        ...
                        filler$.each( function( i ) {
                            ...
                            if ( pageOffset <= start ) {
                                ...
                                remove = self._insertData( out, pageOffset, count, f$, "before" );
                                found = true;
                            } else if ( pageOffset + count === end + 1 || i === filler$.length - 1 ) {
                ...
                if ( o.pagination.scroll && ( self.controlBreakCollapsed ||
                        !hasTotalRecords && dataContainer$.last().height() < self.scrollParent$.height() ) ) {
                    let total = self.model.getTotalRecords();
                    if ( total < 0 || self.pageOffset + self.pageCount < total ) {
                        let needData = true;
                        self.pageOffset += self.pageCount;
                        if ( self.scrollPaging && hasTotalRecords ) {
                            needData = false;
                            // make sure it is not data we already have; find a filler where it belongs
                            filler$ = dataContainer$.last().find( SEL_GRID_SCROLL_FILLER );
                            filler$.each( function() {
                                if ( self.pageOffset === parseInt( $(this).attr( "data-start" ), 10 ) ) {
                                    needData = true;
            ...
            if ( hasControlBreaks ) {
                if ( this.pageOffset === 0 || !o.pagination.scroll ) {
                    ...
                    if ( metaKey ) {
                        ...
                        let meta,
                            rowItem = this.model.recordAt( this.pageOffset - 1 );
                        if ( rowItem ) {
            ...
            if ( !o.pagination.scroll ) {
                ...
                this.pageOffset = pageBoundary( this.pageOffset, this.pageSize );
            }
            pageOffset = this.pageOffset;
            this.renderInProgress = true;
            this.model.forEachInPage( this.pageOffset, pageSize, function( rowItem, index, id ) {
                var meta, expandControl, serverOffset, currentBreakData;
        ...
        _setModelValue: function( element$, columnItem, record, property, notify ) {
            var result, value, prevValue, validity, id, prevId, field;
            ...
            field = this.model.getOption( "fields" )[property];
            if ( field.dataType === "NUMBER" && field.formatMask ) {
                // Remove number formatting if any
                value = toPlainNumber(value, field.formatMask);
            }
        ...
        */
            ...
            if ( targetUrl && col.linkText ) {
                ...
                if ( ( value === null || value === "" ) && ( o.showNullAs || meta.agg || targetUrl ) ) {
                    ...
                    if ( typeof value === "object" && value.hasOwnProperty( "d" ) ) {
                        value = value.d;
                    } else if ( col.dataType === "NUMBER" && col.formatMask ) {
                        // todo server should not need to format this but currently it does
                        value = apex.locale.formatNumber( toPlainNumber(value), col.formatMask );
                    // todo format dates once supported by apex.locale
                    } else {
        ...
        /*
         * Call as part of view refresh. Either via _initPaginationFooter or directly.
         * scrollParent$ is optional when hasSize option is false
         */
        _updateScrollHandler: function( scrollParent$ ) {
            ...
            if ( this.scrollPaging ) {
                if ( prevScrollParent$ && this._scrollHandler ) {
                    prevScrollParent$.off( "scroll", null, this._scrollHandler );
                } else {
                ...
                this.scrollParent$.on( "scroll", null, this._scrollHandler );
            }
        },
            ...
            if ( st !== this.lastScrollTop ) {
                ...
                if ( !this.scrollPageTimer ) {
                    this.scrollPageTimer = setTimeout( function() {
                        var filler$, tbody$, st, h, prevOffset, vpHeight, vpTop, vpBottom, total,
                            o = self.options,
                            rowHeight = self.displayPageHeight / self.displayPageSize;
                        ...
                        if ( autoLoadMore ) {
                            ...
                            h = tbody$.height();
                            st = ( self.scrollParent$[0] === window ? tbody$.offset().top : tbody$.position().top ) - self.scrollParent$.scrollTop();
                            if ( 0 - st + ( vpHeight * 1.5 ) > h ) {
                                total = self.model.getTotalRecords();
                                if ( total < 0 || this.pageOffset + this.pageCount < total ) {
                                    self.pageOffset += self.pageCount; // use pageCount because pageSize can change see bug 28834484.
                                    self._addPageOfRecords();
                                } else {
                                    self.scrollParent$.off( "scroll", null, self._scrollHandler );
                                }
                            }
                        } else {
                            // if the data container has more than one element they should each have the same filter offsets so only need the first one
                            filler$ = self._getDataContainer().first().find( SEL_GRID_SCROLL_FILLER );
                            if ( !filler$.length ) {
                            ...
                            filler$.each(function() {
                                ...
                                top = ( self.scrollParent$[0] === window ? f$.offset().top : f$.position().top ) - self.scrollParent$.scrollTop();
                                bottom = top + fHeight;
                                ...
                                if ( top <= vpBottom && bottom >= vpTop ) {
                                    ...
                                    if ( fHeight < ( vpHeight * 2 ) || top > vpTop  ) {
                                        ...
                                        self.pageOffset = pageBoundary( start + Math.floor( ( fHeight - bottom ) / rowHeight ), self.pageSize );
                                    }

                                    if (self.pageOffset !== prevOffset ) {
                                        // just in case don't add pages for offset already at
                                        // xxx may need to constrain the count so it doesn't go off the end of the filler
                                        self._addPageOfRecords();
                                    }
        ...
        _updateTotalRecords: function() {
            ...
            if ( this.pageCount === 0 && this.pageOffset === 0 ) {
                ...
                this.footer$.find( ".js-pg-prev,.js-pg-next,.js-pg-first,.js-pg-last" ).attr( A_DISABLED, true );
            } else {
                this.footer$.find( ".js-pg-prev,.js-pg-next,.js-pg-first,.js-pg-last" ).attr( A_DISABLED, false );
                this.element.find( ".js-load" ).attr( A_DISABLED, false );
                if ( this.pageOffset === 0 ) {
                    // at the beginning disable prev and first
                    this.footer$.find( ".js-pg-prev,.js-pg-first" ).attr( A_DISABLED, true );
                }
                if ( total > 0 && currentPage === totalPages - 1 ) {
                    this.onLastPage = true;
                    // at the end disable next and last and loadMore
                    this.footer$.find( ".js-pg-next,.js-pg-last" ).attr( A_DISABLED, true );
            ...
            if ( pagination.hideSinglePage && !pagination.scroll ) {
                this.footer$
                    .find( ".js-pg-prev,.js-pg-next,.js-pg-first,.js-pg-last" + ( hasTotalRecords ? "" : ",.js-rangeDisplay" ) )
                    .add( this.pageSelector$ )[totalPages === 1 ? "hide" : "show"]();
            }
            if ( o.hideEmptyFooter ) {
                this.footer$[this.noData ? "hide" : "show"]();
            }
    ...
    $.widget( "apex.tableModelView", $.apex.tableModelViewBase,
        ...
        _eventHandlers: {
            keydown: function( event ) {
                ...
                if ( !o.useIconList || event.isDefaultPrevented() ) {
                    return;
                }
        ...
        refresh: function( pFocus ) {
            ...
            this._addPageOfRecords( function() {
                ...
                if ( selection && selection.length > 0 ) {
                    self.setSelectedRecords( selection, pFocus );
//xxx            } else {
//                if ( pModelChanged ) {
...
//                }
        ...
        setSelection: function( pElements$, pFocus, pNoNotify ) {
            var iconList = this.getIconList();
            if ( iconList ) {
                return iconList.setSelection( pElements$, pFocus, pNoNotify );
            }
        },
        ...
        setSelectedRecords: function( pRecords, pFocus, pNoNotify ) {
            ...
            if ( this.options.useIconList && this.data$ ) {
                for ( i = 0; i < pRecords.length; i++ ) {
                    keys[this.model.getRecordId( pRecords[i] )] = 1;
                }
        ...
        _initRecordTemplate: function() {
            ...
            m = startsWithTag.exec( applyTemplate( o.recordTemplate ) );
            if ( !m ) {
                if ( applyTemplate( o.beforeTemplate ).toLowerCase().match( /<ul|<ol/ ) ) {
        ...
        _initView: function() {
            ...
            m = /<([^ <>]*)/.exec( applyTemplate( o.recordTemplate ) );
            if ( m && m.length > 1 ) {
        ...
        _initRecordMetrics: function() {
            ...
            if ( !r$.length ) {
                r$ = $( applyTemplate( o.recordTemplate ) );
        ...
        _elementsFromRecordIds: function( ids ) {
            ...
            for ( i = 0; i < ids.length; i++ ) {
                item$ = this.data$.children("[data-id='" + ids[i] + "']");
                if ( item$[0] ) {
                    elements.push( item$ );
                }
            }
        ...
        _insertData: function( out, offset, count, filler$, how ) {
            ...
            if ( this.options.useIconList ) {
                vp$ = this.element.find( SEL_TMV_WRAP_SCROLL );
        ...
        _renderRecord: function( out, record, index, id, meta ) {
            var highlight, itemMarkup,
                o = this.options,
                cls = "",
                data = {};

            if ( meta.agg ) {
                // aggregate records are not supported
                return;
            }
...
//                }
            ...
            itemMarkup = applyTemplate( this.options.recordTemplate, {
        ...
        _insertRecord: function( element, record, id, meta, where ) {
            ...
            newItem$ = $( out.toString() );
            if ( where === "after" && element ) {
        ...
        _replaceRecord: function( element, record, oldId, id, meta ) {
            var out = util.htmlBuilder(),
                index; // xxx where does this come from?
