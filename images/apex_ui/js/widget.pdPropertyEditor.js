...
/**
 ...
 Oracle Database Application Express, Release 20.2
    ...
    $.widget( "apex.pdPropertyEditor", {
        ...
        _create: function () {
            ...
            $.apex.propertyEditor.addPropertyType( PROP_TYPE.LINK, {
                ...
                init: function( pElement$, prop, lastComponents ) {
                    ...
                    pElement$.closest( "div." + PROPERTY ).on( "click", "#" + pElement$.attr( "id" ), function() {
                        ...
                        switch ( model.getComponentType( lastComponents[ 0 ].typeId ).id ) {
                            ...
                            case model.COMP_TYPE.BUTTON:
                                lButtonMode = true;
                                break;
                            case model.COMP_TYPE.CARD_ACTION:
                                lCardMode = true;
                                break;
                        ...
                        if ( lButtonMode ) {
                            // Note: Again just using first selected, which is safe to do as links do not yet support multi-edit
                            switch ( lastComponents[ 0 ].getProperty( model.PROP.BUTTON_ACTION ).getValue() ) {
                                case "REDIRECT_URL":
                            ...
                            switch ( lastComponents[ 0 ].getProperty( model.PROP.CARD_LINK_TARGET_TYPE ).getValue() ) {
                                case "REDIRECT_URL":
                                    lTypeValue = pd.LINK_TYPE.URL;
                                    break;
                                case "REDIRECT_PAGE":
                        ...
                        lPropertySet.push (
                            that._getDisplayGroupTarget( [
                                that._getPropertyLinkType({
                                    value:          lTypeValue,
                                    metaData: {
                                        isReadOnly: lLinkReadOnly || lButtonMode || lCardMode,
                ...
                init: function( pElement$, prop, lastComponents ) {
                    ...
                    lModalButton$.on( "click", function openDialog() {
                        var lEditorDlg$, lEditor$, lDlgHeight, lDlgWidth,
                            ...
                            lLanguage = "plaintext",
                            lJavaScriptOptions = { allowNamelessFunctions: false },
                        ...
                        switch ( prop.metaData.type ) {
                            ...
                            case model.PROP_TYPE.JAVASCRIPT:
                                lLanguage = "javascript";
                                if ( prop.propertyName === model.PROP.INIT_JAVASCRIPT_CODE || prop.propertyName === model.PROP.PLUGIN_INIT_JAVASCRIPT_CODE ) {
                                    lJavaScriptOptions.allowNamelessFunctions = true;
                                }
                                break;
                        ...
                        if ( lLanguage === "sql" && !prop.metaData.isReadOnly ) {
                        ...
                        lEditorDlg$ = $( out.toString()).dialog({
                            ...
                            open: function() {
                                ...
                                lEditor$ = $( "#editorDlg-codeEditor" ).codeEditor( $.extend( {
                                    language:          lLanguage,
                                    javascriptOptions: lJavaScriptOptions,
                                    autofocus:         true,
                                    readOnly:          prop.metaData.isReadOnly,
                                    ...
                                    suggestions:  function( pLanguage ) {
            ...
            $.apex.propertyEditor.addPropertyType( PROP_TYPE.TEMPLATE_OPTIONS, {
                ...
                _getDisplayValue: function( prop, lastComponents ) {
                    var lValuesMap,
                        lDisplayValues = [],
                        lValue = prop.value,
                ...
                render: function( out, id, prop, lastComponents ) {
                    var lLabelId = id + "_label",
                        lIsDisabled,
                        lDisplayValue = this._getDisplayValue( prop, lastComponents ),
                        lValueInDom = prop.value;
            ...
            for ( i = 0; i < pComponents.length; i++ ) {
                if ( i === 0 ) {
                    ...
                    if ( lPropertyValue !== pComponents[ i ].getProperty ( pPropertyId ).getValue () ) {
                        lPropertyValue = this.element.propertyEditor ( "getValueVariesConstant" );
        ...
        _prepareMultiSelectMap: function ( pComponents, pPropertyId ) {
            ...
            for ( lCountKey in occurrenceCounts ) {
                if ( occurrenceCounts.hasOwnProperty ( lCountKey ) ) {
        ...
        _clearPropertyEditor: function () {
            ...
            pd.activateTab( "peComponentProperties" );
        ...
        _getCurrentModeComponents: function( pComponents ) {
            var i, lRegionType, lComponentTypeId, lTempComponents, lFilterParentId, lTempFilterComponents,
                ...
                REGION_COMP_TYPE_MAP = {
                    ...
                    NATIVE_FLASH_CHART5: {
                        attributes: model.COMP_TYPE.CHART
                    },
                    NATIVE_FLASH_MAP: {
                        attributes: model.COMP_TYPE.MAP_CHART
                    },
                    NATIVE_IR: {
        ...
        _getPropertyGroups: function ( pComponents, pComponentType ) {
            ...
            if ( pComponentType.isSharedComponent ) {
                lProperties = [ pComponents[ 0 ].getProperty ( pComponentType.displayPropertyId ) ];
            } else {
            ...
            for ( i = 0; i < lProperties.length; i++ ) {
                ...
                lPropertyValue = this._getPropertyValue ( pComponents, lPropertyId );
        ...
        _selectComponents: function ( pComponents, pPropertyId ) {
            ...
            peGoToGroupMenuItems.push({
                    ...
                    action: function () {
                        self.element.propertyEditor( "expandAll" );
                    }
                },
                { type: "separator" }
            );
            for ( k = 0; k < lGroups.length; k++ ) {
                peGoToGroupMenuItems.push({
                    ...
                    action: function () {
                        self.element.propertyEditor( "goToGroup", this.value );
                ...
                model.observer (
                    ...
                    function( pNotifications ) {
                        ...
                        if ( pNotifications[ 0 ].component.parentId  === pComponents[ 0 ].id ) {
                            $( document ).trigger( "selectionChanged", [ self.widgetName, pComponents ] );
                        }
                    },
                    true
                );
                    ...
                    for ( i = 0; i < len; i++ ) {
                        ...
                        for ( lPropertyId in notif.properties ) {
                            ...
                            if ( notif.action === 'redo' ) {
                                self.element.find ( "[data-property-id=" + lPropertyId + "]" ).first ().parent ().parent ().addClass ( IS_CHANGED );
                                ...
                                if ( notif.component.getProperty ( lPropertyId ) !== undefined ) {
                                    ...
                                    if ( !notif.component.getProperty ( lPropertyId ).hasChanged ) {
                                        self.element.find ( "[data-property-id=" + lPropertyId + "]" ).first ().parent ().parent ().removeClass ( IS_CHANGED );
                                    }
                                }
                            }
            ...
            model.observer (
                ...
                function ( pNotifications ) {
                    ...
                    lAffectedProperties = self._getEventSpecificProperties ( pNotifications[ 0 ].properties, [ model.EVENT.ERRORS, model.EVENT.NO_ERRORS, model.EVENT.WARNINGS, model.EVENT.NO_WARNINGS, model.EVENT.META_DATA ] );
