...
//                delete pNode[ "children" ];
    ...
    function addCard( pParentNode, pRegion ) {
    ...
    function addInteractiveReport( pParentNode, pRegion ) {
        ...
        var lComponent = model.getComponents( model.COMP_TYPE.IR_ATTRIBUTES, { parentId: pRegion.id })[ 0 ];
    ...
    function addFacetedSearch( pParentNode, pRegion ) {

        function addFacetGroupChildren( pFacetGroupNode, pFacetGroup ) {

            addComponents( pFacetGroupNode, {
                header: {
        ...
        addComponents( pParentNode, {
            ...
            component: [
                ...
                },
                {
                    ...
                    addSubComponents: addFacetGroupChildren
                }
            ]
        });
    ...
    function addMapChart( pParentNode, pRegion ) {
        ...
        var lComponent = model.getComponents( model.COMP_TYPE.MAP_CHART, { parentId: pRegion.id })[ 0 ];
    ...
    function addChart( pParentNode, pRegion ) {
        ...
        var lComponent = model.getComponents( model.COMP_TYPE.CHART, { parentId: pRegion.id })[ 0 ];
    ...
    function getComponentHierarchyValues( pNode ) {
        ...
        switch ( pNode.type ) {
            ...
            case "facet_group_item":
                ...
                addValue({ id: model.PROP.FACET_REGION, typeId: model.COMP_TYPE.REGION });
                break;
        }
    ...
    function createComponent( pNode, pTree$, pComponentTypeId, pSecondaryType ) {
        ...
        if ( pComponentTypeId === model.COMP_TYPE.REGION ) {
            ...
            lParentComponent = getNearestComponent( model.COMP_TYPE.REGION, pNode ).getChilds( model.COMP_TYPE.CHART )[ 0 ];
            lParentId = lParentComponent.id;
            ...
            lParentId = getNearestComponent( model.COMP_TYPE.REGION, pNode ).getChilds( model.COMP_TYPE.MAP_CHART )[ 0 ].id;

        } else if ( pComponentTypeId === model.COMP_TYPE.FACET ) {

            lParentComponent = getNearestComponent( model.COMP_TYPE.REGION, pNode );
            lParentId        = lParentComponent.id;

        } else if ( pComponentTypeId === model.COMP_TYPE.FACET_GROUP ) {

            lParentComponent = getNearestComponent( model.COMP_TYPE.REGION, pNode );
            lParentId        = lParentComponent.id;

        } else if ( pComponentTypeId === model.COMP_TYPE.FACET_GROUP_ITEM ) {

            lParentComponent = getNearestComponent( model.COMP_TYPE.FACET_GROUP, pNode );
            lParentId        = lParentComponent.id;
            addValue({ id: model.PROP.FACET_REGION, typeId: model.COMP_TYPE.REGION });
    ...
    function getTreeRules() {
        ...
        function canAddChartSeries( pNode ) {
            ...
            return ( $.inArray(
                ...
                getNearestComponent( model.COMP_TYPE.REGION, pNode ).getChilds( model.COMP_TYPE.CHART )[ 0 ].getProperty( model.PROP.CHART_TYPE ).getValue(),
        ...
        return {
            ...
            "region": {
                ...
                validChildren: [
                    "page_items", "region_buttons", "inline_da_events", "region_columns", "ig_columns", "chart_series", "map_chart_series", "svg_chart_series", 
                    "jet_chart_series", "jet_chart_axes", "facets", "ir_column_groups", "ir_saved_reports", "ig_column_groups", "ig_saved_reports",
    ...
    function getTreeNodeAdapter( pDataFunction, pExpandedState ) {
        ...
        lNodeAdapter.getIcon = function( n ) {
            ...
            if ( n.data ) {
                if ( n.data.typeId === model.COMP_TYPE.REGION ) {
                ...
                } else if ( n.data.typeId === model.COMP_TYPE.FACET ) {
                    return pd.getComponentIconClass( "item", getComponent( n ).getProperty( model.PROP.ITEM_TYPE ).getValue() );
                } else if ( n.data.typeId === model.COMP_TYPE.FACET_GROUP_ITEM ) {
                    return pd.getComponentIconClass( "item", getComponent( n ).getProperty( model.PROP.ITEM_TYPE ).getValue() );
                    ...
                    return pd.getComponentIconClass( "card-action", getComponent( n ).getProperty( model.PROP.CARD_ACTION_TYPE ).getValue() );
                } else if ( n.data.typeId === model.COMP_TYPE.CLASSIC_RPT_COLUMN ) {
                    return pd.getComponentIconClass( "col", getComponent( n ).getProperty( model.PROP.CLASSIC_REPORT_COLUMN_TYPE ).getValue() );
                } else if ( n.data.typeId === model.COMP_TYPE.IR_COLUMN ) {
                    return pd.getComponentIconClass( "col", getComponent( n ).getProperty( model.PROP.IR_COLUMN_DISPLAY_TYPE ).getValue() );
            ...
            if ( pResult ) {
                ...
                if ( $.inArray( pRule, [ "canDelete", "canRename", "canDrag", "canDuplicate" ]) !== -1 ) {
                    ...
                    // Make sure children can only be moved within parent
                          ...
                          && pOperation === "move"
                          && $.inArray( pNode.type, [ "region_columns", "ig_columns", "ir_column_groups", "ig_column_groups", "facets", "chart_series", "map_chart_series","jet_chart_series", "jet_chart_axes" ]) !== -1
                          && pChildren
                          )
                {
                ...
                } else if ( pRule === "canAdd" && pOperation === "move" && pNode.type === "facet_group_items" && pChildren ) {
                    // Make sure facet group icons can only be moved within parent Faceted Search region
                    for ( i = 0; i < pChildren.length; i++ ) {
        ...
        function addSynchronize( pLabel, pRegionType ){
            ...
            addItem( {
                ...
                visible: function (){
                    ...
                    if ( !model.isPageReadOnly() && isNotGlobal( this, ["region", "ig_columns", "region_columns", "page_items"] )) {
                        lRegion = getNearestComponent( model.COMP_TYPE.REGION, this[0], pTree$ );
