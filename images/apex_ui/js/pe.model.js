...
//            PLUGIN:           "1", // todo to be changed to real number
                ...
                if ( pTypeId !== COMP_TYPE.REGION ) {
                    ...
                    for ( var j = 0; j < pPlugins[ i ].attributes.length; j++ ) {
                           ...
                           || ( pTypeId === COMP_TYPE.REGION_COLUMN && lAttribute.scope === "COLUMN" ))
                        {
                            ...
                            if ( pTypeId !== COMP_TYPE.IG_COLUMN && pTypeId !== COMP_TYPE.FACET && pTypeId !== COMP_TYPE.FACET_GROUP_ITEM ) {
                    ...
                    if ( pTypeId !== COMP_TYPE.REGION_COLUMN && pTypeId != COMP_TYPE.IG_COLUMN && pTypeId != COMP_TYPE.FACET && pTypeId !== COMP_TYPE.FACET_GROUP_ITEM ) {
    ...
    function generateCrossReferences() {
        ...
        for ( var i in gProperties ) {
            if ( gProperties.hasOwnProperty( i )) {
                ...
                } else if ( gProperties[ i ].type === PROP_TYPE.ITEM ) {
    ...
    function isValidPlugin( pComponent, pPlugin, pCheckOptions ) {
        ...
        return (  ( !pCheckOptions.ui || isUiSupported( pComponent, pPlugin.uiTypes ))
               && ( !pPlugin.componentTypes || $.inArray( pComponent.typeId, pPlugin.componentTypes) !== -1 )
    ...
    function replaceDefaultPlaceholders( pProperty, pDefaultValue ) {
        ...
        if ( lValue.indexOf( "#" ) !== -1 ) {
            ...
            if ( gSharedComponents.codeLanguages.hasOwnProperty( CURRENT_CODE_LANGUAGE )) {
        ...
        if ( lPluginType && this.component._properties.hasOwnProperty( lPluginType.typePropertyId )) {
            ...
            if ( lPlugin ) {
                ...
                if ( PLUGIN_DYNAMIC_PROPERTIES_MAP.hasOwnProperty( this.id )) {
                    ...
                    if (  lPlugin.stdAttributes && lPlugin.stdAttributes.hasOwnProperty( lStdAttributeName ) && this.id !== PROP.LOV_TYPE && this.id !== PROP.FACET_LOV_TYPE ){
                ...
                if ( lPropertyDef.lovType === "PLUGINS" ) {
                    ...
                    if ( gTypes[ this.component.typeId ].pluginType.plugins.hasOwnProperty( lValue )) {
                        return formatPostfix(
                            ...
                            !isUiSupported( this.component, gTypes[ this.component.typeId ].pluginType.plugins[ lValue ].uiTypes ));
                    }
        ...
        if ( lPropertyDef.type === PROP_TYPE.SUPPORTED_UI ) {
            ...
            if ( lPropertyDef.lovType === "PLUGINS" ) {
                ...
                forEachAttribute( lType.pluginType.plugins, function( pPluginName, pPlugin ) {
                    ...
                    if ( lSelf._value === pPluginName || isValidPlugin( lSelf.component, pPlugin, { isDeprecated: true } )) {
                        lIsSupported = isUiSupported( lSelf.component, pPlugin.uiTypes );
                        lLovValue    = {
                            r:           pPluginName,
            ...
            if ( lPropertyDef.lovType === "COMPONENT" ) {
                ...
                if ( lPropertyDef.lovComponentScope === "PARENT" ) {
                    ...
                    if ( this.id === PROP.FACET_GROUP ) {
                        lFilter = {parentId: this.component._properties[PROP.FACET_REGION]._value, excludeGlobalPage: true};
                    } else if ( this.id === PROP.DEPENDING_ON_FACET_GRP_ITEM ) {
                        lFilter.properties = [{
                            id:    PROP.FACET_REGION,
                            value: this.component._properties[PROP.FACET_REGION]._value
                        }];
                    } else if ( this.id === PROP.DEPENDING_ON_FACET ) {
                        lFilter = {parentId: lSelf.component.parentId, excludeGlobalPage: true};
                    }
                ...
                // Faceted Search is currently only available for Classic Report and Cards
                if ( lPropertyDef.id === PROP.SOURCE_FILTERED_REGION ) {
                    lFilter.properties = [{
                        id:    PROP.REGION_TYPE,
                        value: new RegExp( "(NATIVE_SQL_REPORT|NATIVE_CARDS)" )
                    }];
                }
                ...
                if ( lSelf.id === PROP.AFFECTED_TYPE ) {
                    ...
                    for ( var i = 0; i < lPropertyDef.lovValues.length; i++ ) {
                        if ( gSharedComponents.codeLanguages.hasOwnProperty( lPropertyDef.lovValues[ i ].r ) || lSelf._value === lPropertyDef.lovValues[ i ].r ) {
                            lLovValues.push( lPropertyDef.lovValues[ i ] );
                        }
                    }
        ...
        if ( lPropertyDef.type === PROP_TYPE.SUPPORTED_UI ) {
            ...
            if ( lPropertyDef.lovType === "PLUGINS" ) {
                ...
                forEachAttribute( lType.pluginType.plugins, function( pPluginName, pPlugin ) {
                    ...
                    if ( pPlugin.isQuickPick && isValidPlugin( lSelf.component, pPlugin, { ui: true, isDeprecated: true, isLegacy: true } )) {
                        addQuickPick({
                            r: pPluginName,
                            d: pPlugin.title
                        });
    ...
    Property.prototype._getHelpText = function() {
        ...
        function getPluginsHelpText( pType ) {
            ...
            lPlugins = $.map( lType.pluginType.plugins, function( pPluginDef, pPluginName ) {
                if ( pPluginName.indexOf( pType ) === 0 && isValidPlugin( lSelf.component, pPluginDef, { ui: true, isDeprecated: true, isLegacy: true } )) {
                    return pPluginDef;
                }
            }).sort( function( a, b ) {
                ...
                PROP_TYPE.TEMPLATE_OPTIONS ]) === -1 )
        {
            ...
            if ( lTypePropertyDef.supportsTemplateDirective ) {
                lHelpText += "<li>" + format( "HELP.TEMPLATE_DIRECTIVE" ) + "</li>";
            }
    ...
    Property.prototype.wsmHasLocalTable = function() {

        return ( this.errors.length === 0 && this._wsmHasLocalTable );
            ...
            if ( $.trim( lValue ) !== "" ) {
                ...
                if ( lPropertyDef.restrictedChars && RESTRICTED_CHARS[ lPropertyDef.restrictedChars ].test( lValue )) {
                    ...
                    )
                    {
                        ...
                        } else if (  $.inArray( lSelf.id, [ PROP.SOURCE_QUERY_PLSQL_FUNC_BODY_SQL,
                                                            PROP.SOURCE_QUERY_JS_FUNC_BODY_SQL,
                                                            PROP.SOURCE_QUERY_JAVA_FUNC_BODY_SQL,
                                                            PROP.SOURCE_QUERY_PYTHON_FUNC_BODY_SQL ] ) !== -1
                        ...
                        } else if (  $.inArray( lPropertyDef.type, [ PROP_TYPE.TABLE, PROP_TYPE.WHERE_CLAUSE, PROP_TYPE.ORDER_BY_CLAUSE,
                                  ...
                                  || this.id === PROP.SOURCE_WEB_SRC_QUERY )
                        {
                            ...
                            if ( $.inArray ( lSelf.id, [ PROP.SOURCE_SQL_QUERY,
                                                         PROP.SOURCE_QUERY_PLSQL_FUNC_BODY_SQL,
                                                         PROP.SOURCE_QUERY_JS_FUNC_BODY_SQL,
                                                         PROP.SOURCE_QUERY_JAVA_FUNC_BODY_SQL,
                                                         PROP.SOURCE_QUERY_PYTHON_FUNC_BODY_SQL,
                                                         ...
                                                         PROP.SOURCE_QUERY_ORDER_BY ]) !== -1 )
                            {
                                ...
                                lFunctionBody       = getPropertyValueIfExists([
                                                          PROP.SOURCE_QUERY_PLSQL_FUNC_BODY_SQL,
                                                          PROP.SOURCE_QUERY_JS_FUNC_BODY_SQL,
                                                          PROP.SOURCE_QUERY_JAVA_FUNC_BODY_SQL,
                                                          PROP.SOURCE_QUERY_PYTHON_FUNC_BODY_SQL]);
                                ...
                                lLocation = getPropertyValueIfExists( [ PROP.SOURCE_LOCATION, PROP.CHART_SOURCE_LOCATION ] );
                            } else if ( $.inArray( this.id, [ PROP.POST_PROC_WHERE,
                                                              PROP.POST_PROC_ORDER_BY,
                                                              PROP.POST_PROC_SQL_QUERY,
                                                              PROP.POST_PROC_PLSQL_FUNC_BODY_SQL,
                                                              PROP.POST_PROC_JS_FUNC_BODY_SQL,
                                                              PROP.POST_PROC_JAVA_FUNC_BODY_SQL,
                                                              PROP.POST_PROC_PYTHON_FUNC_BODY_SQL ]) !== -1 )
                            {
                                lLocation           = getPropertyValueIfExists( [ PROP.SOURCE_LOCATION, PROP.CHART_SOURCE_LOCATION ] );
                            ...
                            ajax (
                                "validateProperty", {
                                    ...
                                    x07: lQueryType,
                                    x08: lTableOwner,
                                    x09: lTableName + lSqlQuery + lFunctionBody + lWebSourceQueryId, // only one will be populated
                                    ...
                                    success: function( pData ) {
                                        ...
                                        lResult.wsmHasLocalTable = pData.webSourceHasLocalTable;
                                        ...
                                        if ( pData.result !== "OK" ) {
                                            ...
                                            if (  (  ( lPropertyDef.type === PROP_TYPE.TABLE || $.inArray( lPropertyDef.type, PROP_TYPES_RETURNING_SQL ) !== -1 )
                                                  && lTypePropertyDef.refByChilds.length > 0 )
                                               || $.inArray( lSelf.id, [ PROP.REGION_FUNCTION_RETURNING_SQL,
                                                                         PROP.SOURCE_WEB_SRC_QUERY,
                                                                         PROP.SOURCE_QUERY_PLSQL_FUNC_BODY_SQL,
                                                                         PROP.SOURCE_QUERY_JS_FUNC_BODY_SQL,
                                                                         PROP.SOURCE_QUERY_JAVA_FUNC_BODY_SQL,
                        ...
                        if ( lTypePropertyDef.checkUniqueness ) { // todo
                            ...
                            if ( lComponents.length > 0 && lComponents[ 0 ].id !== this.component.id ) {
                                ...
                                // Item names (used for page items and facets) have to be unique within those component types
                                lFilters = [];
    ...
    Property.prototype._setValue = function( pValue, pOptions ) {
        ...
        function autoCorrect() {
            ...
            // 1) Auto trim value
            if ( $.inArray( lPropertyDef.type, [ PROP_TYPE.NUMBER, PROP_TYPE.INTEGER, PROP_TYPE.LINK, PROP_TYPE.ITEM, PROP_TYPE.FACET, PROP_TYPE.PAGE, PROP_TYPE.TABLE ]) !== -1 ) {
                lValue = $.trim( lValue );
            }
        ...
        function renamePageItemReferences( pOldName, pNewName ) {
            ...
            for ( i = 0; i < gTypes[ COMP_TYPE.PAGE_ITEM ].refByProperties.length; i++ ) {
                ...
                if ( lRefPropertyDef.type === PROP_TYPE.ITEM || lRefPropertyDef.type === PROP_TYPE.FACET ) {
        ...
        lSelf._wsmHasLocalTable = lValidationResult.wsmHasLocalTable;
                ...
                if ( lType.id === COMP_TYPE.REGION && ( lAction === CALLBACK_ACTION.CREATED || lAction === CALLBACK_ACTION.REMOVED )) {
                    ...
                    if ( lAction === CALLBACK_ACTION.CREATED && $.inArray( lPlugin.name, [ "NATIVE_SQL_REPORT", "NATIVE_TABFORM" ]) === -1 ) {
                        ...
                        if (  lPluginDef.attributes.length > 0
                            ...
                            || $.inArray( "EDITABLE",              lPluginDef.features ) !== -1
                            || $.inArray( "LAZY_LOADING",          lPluginDef.features ) !== -1
...
*/
    ...
    Component.prototype._remove = function( pChildDelete ) {
        ...
        if ( this.typeId === COMP_TYPE.PAGE_ITEM || this.typeId === COMP_TYPE.FACET || this.typeId === COMP_TYPE.FACET_GROUP_ITEM ) {
        ...
        for ( var i = 0; i < lType.refByProperties.length; i++ ) {
            ...
            } else if ( lPropertyDef.type === PROP_TYPE.ITEM || lPropertyDef.type === PROP_TYPE.FACET ) {
                if ( lPropertyDef.multiValueDelimiter ) {
                ...
                if ( gTypes[ lPropertyDef.refByComponentTypes[ j ]].parentId !== this.typeId ) {
                    ...
                    for ( var k = 0; k < lRefComponents.length; k++ ) {
                        if ( lTypePropertyDef.referenceOnDelete === "WEAK_NULL" || lTypePropertyDef.referenceOnDelete === "DB_NULL" ) {
                            ...
                            if (( lPropertyDef.type === PROP_TYPE.ITEM || lPropertyDef.type === PROP_TYPE.FACET ) && lPropertyDef.multiValueDelimiter ) {
    ...
    Component.prototype.getSiblings = function( pIncludeSelf ) {
        ...
        if ( $.inArray( this.typeId, [ COMP_TYPE.REGION, COMP_TYPE.PAGE_ITEM, COMP_TYPE.BUTTON, COMP_TYPE.FACET, COMP_TYPE.FACET_GROUP ]) !== -1 ) {
            ...
            } else if ( this.typeId === COMP_TYPE.FACET || this.typeId === COMP_TYPE.FACET_GROUP ) {
                lFilters.push({
    ...
    function getItemsLov( pFilters, pCallback ) {
        ...
        if ( pFilters.type === "application" ) {
            ...
            lComponents = getComponents( COMP_TYPE.FACET, { pageId: gCurrentPageId, filterFunction: function() {
    ...
    function getNotifications( pOldComponent, pNewComponent, pAction ) {
        ...
        function addComponentPropertyNotification( pComponent, pPropertyId, pEvent ) {
            if ( pComponent._properties.hasOwnProperty( pPropertyId ) || pEvent === EVENT.REMOVE_PROP ) {
                           ...
                           )
                        {
                            for ( var i = 0; i < lTypeDef.properties[ lPropertyId ].refByChilds.length; i++ ) {
                                if ( lTypeDef.properties[ lPropertyId ].refByChilds[ i ].typeId === pNewComponent.typeId ) {
                                    ...
                                    if ( !lChildComponentTypes.hasOwnProperty( lChildTypeId )) {
        ...
        for ( var lChildTypeId in lChildComponentTypes ) {
            if ( lChildComponentTypes.hasOwnProperty( lChildTypeId )) {
                ...
                for ( var i = 0; i < lChildComponents.length; i++ ) {
                    ...
                    for ( var j = 0; j < lChildComponentTypes[ lChildTypeId ].length; j++ ) {
    ...
    return {
        ...
        NO_NODE_COMP_TYPES:    NO_NODE_COMP_TYPES,
        SOURCE_SNIPPET_PROPS:  SOURCE_SNIPPET_PROPS,
        PROP_TYPE:        PROP_TYPE,
        EVENT:            EVENT,
        ...
        getPluginProperty: function( pComponentTypeId, pPlugin, pAttributeNo ) {
            ...
            } else if ( pComponentTypeId === COMP_TYPE.IG_COLUMN || pComponentTypeId === COMP_TYPE.FACET || pComponentTypeId === COMP_TYPE.FACET_GROUP_ITEM ) {
                // Interactive Grid column or Facet plug-ins are item type plug-ins which are stored for page items
